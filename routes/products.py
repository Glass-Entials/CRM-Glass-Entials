...
def products_list():
    ...
    active_products = Product.query.filter_by(
        organization_id=org_id, is_deleted=False, status=ProductStatus.ACTIVE
    ).count()
    low_stock_count = sum(
        1
        for p in Product.query.filter_by(organization_id=org_id, is_deleted=False).all()
        if p.is_low_stock
    )
    ...
    if request.method == "POST":
        ...
        try:
            product = Product(
                ...
                min_price=(
                    float(request.form.get("min_price"))
                ...
                min_stock_alert=(
                    float(request.form.get("min_stock_alert"))
                    if request.form.get("min_stock_alert")
                    else None
                ),
            ...
            flash("An error occurred. Please try again.", "producterror")
            return redirect(url_for("products.add_product"))
...
def view_product(product_id):
    ...
    product = Product.query.filter_by(
        id=product_id, organization_id=org_id, is_deleted=False
    ).first_or_404()
...
def edit_product(product_id):
    ...
    if request.method == "POST":
        ...
        product.category = cat_map.get(
            request.form.get("category"), ProductCategory.OTHER
        )
        product.status = status_map.get(
            request.form.get("status"), ProductStatus.ACTIVE
        )
...
def adjust_stock(product_id):
    ...
    try:
        ...
        flash(
            f'Stock {action} for "{product.name}". New qty: {product.stock_quantity}',
            "productsuccess",
        )
    except Exception as e:
        db.session.rollback()
