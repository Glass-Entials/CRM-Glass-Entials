...
def generate_invoice_number(org_id):
    """Generate next Invoice number atomically using row-level lock."""
    prefix = "INV"
    ...
    if not settings:
        settings = QuotationSettings(organization_id=org_id, invoice_counter=1)
        db.session.add(settings)
        db.session.flush()
    else:
        current_val = settings.invoice_counter or 0
        settings.invoice_counter = current_val + 1
        db.session.flush()
    return f"{prefix}/{year}/{settings.invoice_counter:04d}"


def _calc_invoice_totals(
    items_data,
    total_discount,
    total_discount_type,
    additional_charges,
...
def _save_invoice_from_form(invoice, form, org_id, employee_id):
    ...
    invoice.customer_id = tenant_record_id(
        Customer, form.get("customer_id"), org_id, allow_none=False, is_deleted=False
    )
    invoice.project_id = tenant_record_id(
...
def view_invoice(invoice_id):
    ...
    settings = _get_or_create_settings(org_id)
    return render_template(
        "accounts/invoice_view.html", invoice=invoice, settings=settings
    )
