...
def register():
    if request.method == "POST":
        ...
        role_pref_mapped = (
            UserRole.MANAGER if role_pref == "manager" else UserRole.EMPLOYEE
        )
...
def user_profile():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            db.session.rollback()
            current_app.logger.error(f"Error: {str(e)}", exc_info=True)
            flash("An error occurred. Please try again.", "profileerror")

        return redirect(url_for("auth.user_profile"))

    my_tasks = (
        Task.query.filter_by(assigned_to=employee.id, organization_id=org_id)
        .order_by(Task.created_at.desc())
        .all()
    )
    my_leads = (
        Lead.query.filter_by(
            assigned_to=employee.id, organization_id=org_id, is_deleted=False
        )
    ...
    my_activities = (
        LeadActivity.query.filter_by(created_by=employee.id, organization_id=org_id)
...
def remove_profile_pic():
    ...
    if employee and employee.profile_pic:
        file_path = os.path.join(
            current_app.config["UPLOAD_FOLDER"], "profile_pics", employee.profile_pic
        )
