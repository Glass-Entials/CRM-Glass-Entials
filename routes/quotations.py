...
def generate_quotation_number(org_id):
    """Generate next GL quotation number atomically using row-level lock."""
    # SELECT ... FOR UPDATE prevents duplicate numbers under concurrent requests
    settings = (
        QuotationSettings.query.filter_by(organization_id=org_id)
        .with_for_update()
        .first()
    )
    if not settings:
        settings = QuotationSettings(organization_id=org_id, number_counter=1)
        db.session.add(settings)
        db.session.flush()
        seq = 1
    else:
        seq = settings.number_counter or 1
...
def _calc_totals(
    ...
    for it in items_data:
        ...
        items_out.append(
            {**it, "amount": amount, "gst_amount": gst_amount, "total": total_item}
        )
        subtotal += amount
        total_quantity += qty
    ...
    total_amount = (
        taxable_base + gst_amount + (0 if additional_charges_taxable else charges)
    )
...
def _save_quotation_from_form(quotation, form, files, org_id, employee_id):
    ...
    quotation.issue_date = (
        datetime.datetime.strptime(issue_date_str, "%Y-%m-%d")
        if issue_date_str
        else datetime.datetime.now()
    )
    quotation.due_date = (
        datetime.datetime.strptime(due_date_str, "%Y-%m-%d") if due_date_str else None
    )
    ...
    # ── Links ───────────────────────────────────────────────
    quotation.customer_id = tenant_record_id(
        Customer, form.get("customer_id"), org_id, is_deleted=False
    )
    quotation.lead_id = tenant_record_id(
        Lead, form.get("lead_id"), org_id, is_deleted=False
    )
    quotation.project_id = tenant_record_id(
        Project, form.get("project_id"), org_id, is_deleted=False
    )
        ...
        items_data.append(
            {
                ...
                "formula_type": (
                    formula_types[i].strip() if i < len(formula_types) else "standard"
                ),
                "quantity": (
                ...
                "discount": (
                    float(discounts[i]) if i < len(discounts) and discounts[i] else 0
                ),
                "discount_type": (
                    discount_types[i] if i < len(discount_types) else "flat"
                ),
                "gst_percentage": (
    ...
    upload_dir = os.path.join(
        current_app.root_path, "static", "uploads", "quotation_attachments"
    )
    os.makedirs(upload_dir, exist_ok=True)
    for af in attach_files:
        if af and af.filename:
            original_name, ext, file_size = validate_upload(af, ATTACHMENT_EXTENSIONS)
    ...
    for idx, tid in enumerate(attached_term_ids):
        try:
            term = QuotationTerm.query.filter_by(
                id=int(tid), organization_id=org_id
            ).first()
            if term:
    ...
    for fid, fval in zip(custom_field_ids, custom_field_vals):
        try:
            field_id = tenant_record_id(
                QuotationCustomField, fid, org_id, allow_none=False
            )
...
def add_quotation():
    ...
    if request.method == "POST":
        try:
            ...
            _save_quotation_from_form(
                quotation, request.form, request.files, org_id, emp.id
            )
            ...
            db.session.commit()
            flash(
                f"✅ {quotation.doc_type_display} {quotation.quotation_number} created!",
                "success",
            )
            return redirect(
    ...
    custom_fields = (
        QuotationCustomField.query.filter_by(organization_id=org_id, is_active=True)
        .order_by(QuotationCustomField.sort_order)
        .all()
    )
    term_groups = (
        QuotationTermGroup.query.filter_by(organization_id=org_id, is_active=True)
        .order_by(QuotationTermGroup.sort_order)
        .all()
    )
...
def edit_quotation(quotation_id):
    ...
    quotation = Quotation.query.filter_by(
        id=quotation_id, organization_id=org_id, is_deleted=False
    ).first_or_404()
...
def download_attachment(att_id):
    att = QuotationAttachment.query.filter_by(
        id=att_id, organization_id=current_user.organization_id
    ).first_or_404()
    path = os.path.join(
