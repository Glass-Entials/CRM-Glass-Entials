...
def view_lead(lead_id):
    ...
    follow_ups = (
        LeadFollowUp.query.filter_by(lead_id=lead_id)
...
def add_lead():
    if request.method == "POST":
        ...
        if existing:
            flash(
                "Lead already exists with this email or phone number in your organization.",
                "leadserror",
            )
            return redirect(url_for("leads.add_lead"))
...
def export_leads(format):
    ...
    if format == "csv":
        ...
        return export_to_pdf(data, headers, filename, title="Leads Management Report")
...
@leads_bp.route("/leads/follow-ups")
@login_required
def all_follow_ups():
    """Global overview of all pending and upcoming follow-ups."""
    org_id = current_user.organization_id
    ...
    # Recent completed (last 15)
    completed = (
        LeadFollowUp.query.join(Lead)
        .filter(
            LeadFollowUp.organization_id == org_id,
            LeadFollowUp.is_done == True,
            Lead.is_deleted == False,
        )
        .order_by(LeadFollowUp.updated_at.desc())
        .limit(15)
        .all()
    )
...
@leads_bp.route("/lead/<int:lead_id>/follow-up/add", methods=["POST"])
@login_required
def add_follow_up(lead_id):
    ...
    method_val = request.form.get("method", "Call")
    outcome_val = request.form.get("outcome", "")
    notes = request.form.get("notes", "").strip()
    follow_date = request.form.get("follow_up_date", "").strip()
    next_date = request.form.get("next_follow_up_date", "").strip()

    if not follow_date:
        flash("Follow-up date is required.", "leadserror")
        return redirect(url_for("leads.view_lead", lead_id=lead_id, tab="followups"))

    try:
        method_map = {e.value: e for e in FollowUpMethod}
        outcome_map = {e.value: e for e in FollowUpOutcome}

        fu = LeadFollowUp(
            lead_id=lead.id,
            method=method_map.get(method_val, FollowUpMethod.CALL),
            outcome=outcome_map.get(outcome_val) if outcome_val else None,
            notes=notes,
            follow_up_date=datetime.strptime(follow_date, "%Y-%m-%dT%H:%M"),
            next_follow_up_date=(
                datetime.strptime(next_date, "%Y-%m-%dT%H:%M") if next_date else None
            ),
            is_done=False,
            created_by=current_user.employee.id,
            organization_id=current_user.organization_id,
        ...
        if lead.assigned_to and lead.assigned_to != current_user.employee.id:
            create_notification(
                recipient_id=lead.assigned_to,
                title="Follow-up Scheduled",
                message=f"A follow-up has been scheduled for lead '{lead.name}'.",
                link=url_for("leads.view_lead", lead_id=lead.id),
                sender_id=current_user.employee.id,
                organization_id=current_user.organization_id,
            )
        ...
        db.session.commit()
        flash("Follow-up scheduled successfully!", "leadssuccess")
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Error: {str(e)}", exc_info=True)
        flash("An error occurred. Please try again.", "leadserror")

    return redirect(url_for("leads.view_lead", lead_id=lead_id) + "#followups")


@leads_bp.route("/lead/follow-up/<int:fu_id>/done", methods=["POST"])
@login_required
def mark_follow_up_done(fu_id):
    fu = LeadFollowUp.query.filter_by(
        id=fu_id, organization_id=current_user.organization_id
    ).first_or_404()
    outcome_val = request.form.get("outcome", "")
    notes = request.form.get("notes", "").strip()
    ...
    fu.is_done = True
    if outcome_val:
        fu.outcome = outcome_map.get(outcome_val, fu.outcome)
    if notes:
        fu.notes = notes
