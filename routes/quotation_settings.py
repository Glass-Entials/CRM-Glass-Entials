...
from utils.security import require_roles, validate_upload
import os, uuid, json

IMAGE_EXTENSIONS = {"png", "jpg", "jpeg", "gif", "webp"}

quotation_settings_bp = Blueprint(
    "quotation_settings", __name__, url_prefix="/quotation-settings"
)
...
def _seed_default_custom_fields(org_id):
    ...
    for label, key, ftype, order in defaults:
        exists = QuotationCustomField.query.filter_by(
            organization_id=org_id, field_key=key
        ).first()
        if not exists:
...
def settings_home():
    ...
    if request.method == "POST":
        ...
        if section == "company":
            ...
            settings.number_prefix = (
                request.form.get("number_prefix", "GL").strip().upper()
            )
            ...
            settings.default_gst_rate = float(
                request.form.get("default_gst_rate") or 18
            )
            ...
            settings.default_payment_terms = (
                request.form.get("default_payment_terms") or None
            )
            settings.default_delivery_terms = (
                request.form.get("default_delivery_terms") or None
            )
            ...
            settings.show_bank_details_on_pdf = (
                request.form.get("show_bank_details_on_pdf") == "1"
            )
            settings.show_signature_on_pdf = (
                request.form.get("show_signature_on_pdf") == "1"
            )
    ...
    custom_fields = (
        QuotationCustomField.query.filter_by(organization_id=org_id)
        .order_by(QuotationCustomField.sort_order)
        .all()
    )
    term_groups = (
        QuotationTermGroup.query.filter_by(organization_id=org_id)
        .order_by(QuotationTermGroup.sort_order)
        .all()
    )

    return render_template(
        "accounts/quotation_settings.html",
        settings=settings,
...
def add_custom_field():
    ...
    max_order = (
        db.session.query(db.func.max(QuotationCustomField.sort_order))
    ...
    db.session.commit()
    return jsonify(
        {"success": True, "id": f.id, "label": f.label, "field_key": f.field_key}
    )
...
def update_custom_field(field_id):
    ...
    f = QuotationCustomField.query.filter_by(
        id=field_id, organization_id=org_id
    ).first_or_404()

    action = request.form.get("action") or request.get_json(
...
def add_term_group():
    ...
    max_order = (
        db.session.query(db.func.max(QuotationTermGroup.sort_order))
        .filter_by(organization_id=org_id)
        .scalar()
        or 0
    )
...
def add_term():
    ...
    max_order = (
        db.session.query(db.func.max(QuotationTerm.sort_order))
